    ...
    public class TestExtensions
    {
        ...
        public void Functions(string extensionFile)
        {
            using (var connection = new SQLiteConnection("Data Source=:memory:"))
            {
                ...
                }
                {
                    ...
                    Assert.Equal(5, (long)command.ExecuteScalar()!); // System.Data.SQLite supports only Int64
                }
                {
                    var command = connection.CreateCommand();
        ...
        public void ChinookDB(string extensionFile)
        {
            using (var connection = new SQLiteConnection("Data Source=chinook.db"))
            {
...

                {
                    ...
                    Assert.Equal("2009-04-22 00:00:00", ((DateTime)command1.ExecuteScalar()!).ToString("yyyy-MM-dd HH:mm:ss.FFF", CultureInfo.InvariantCulture));
        ...
        public void NorthwindDB(string extensionFile)
        {
            using (var connection = new SQLiteConnection("Data Source=northwind.db"))
            {
...

                {
                    ...
                    Assert.Equal("2014-11-15 00:00:00", ((DateTime)command1.ExecuteScalar()!).ToString("yyyy-MM-dd HH:mm:ss.FFF", CultureInfo.InvariantCulture));
        ...
        public void Tables(string extensionFile)
        {
            using (var connection = new SQLiteConnection("Data Source=:memory:"))
            {
                ...
                {
                    var command1 = connection.CreateCommand();
                    command1.CommandText = @"CREATE VIRTUAL TABLE RecordParamsTable USING MyRecordParamsTable(name1, 100)";
                    Assert.Equal(0, command1.ExecuteNonQuery());

                    var command2 = connection.CreateCommand();
                    command2.CommandText = @"SELECT * FROM RecordParamsTable";
                    using (var reader = command2.ExecuteReader())
                    {
                        Assert.True(reader.Read());
                        Assert.Equal("name1", (string)reader["Name"]);
